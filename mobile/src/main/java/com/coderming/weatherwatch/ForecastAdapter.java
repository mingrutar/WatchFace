...
import com.coderming.weatherwatch.data.WeatherContract;
...
public class ForecastAdapter extends RecyclerView.Adapter<ForecastAdapter.ForecastAdapterViewHolder> {
    ...
    final private com.coderming.weatherwatch.ItemChoiceManager mICM;
    ...
    public ForecastAdapter(Context context, ForecastAdapterOnClickHandler dh, View emptyView, int choiceMode) {
        ...
        mICM = new com.coderming.weatherwatch.ItemChoiceManager(this);
    ...
    public void onBindViewHolder(ForecastAdapterViewHolder forecastAdapterViewHolder, int position) {
        ...
        int weatherId = mCursor.getInt(com.coderming.weatherwatch.ForecastFragment.COL_WEATHER_CONDITION_ID);
        int defaultImage;
        boolean useLongToday;

        switch (getItemViewType(position)) {
            case VIEW_TYPE_TODAY:
                defaultImage = com.coderming.weatherwatch.Utility.getArtResourceForWeatherCondition(weatherId);
                useLongToday = true;
                break;
            default:
                defaultImage = com.coderming.weatherwatch.Utility.getIconResourceForWeatherCondition(weatherId);
                useLongToday = false;
        }

        if ( com.coderming.weatherwatch.Utility.usingLocalGraphics(mContext) ) {
            forecastAdapterViewHolder.mIconView.setImageResource(defaultImage);
        } else {
            Glide.with(mContext)
                    .load(com.coderming.weatherwatch.Utility.getArtUrlForWeatherCondition(mContext, weatherId))
                    .error(defaultImage)
                    .crossFade()
        ...
        long dateInMillis = mCursor.getLong(com.coderming.weatherwatch.ForecastFragment.COL_WEATHER_DATE);

        // Find TextView and set formatted date on it
        forecastAdapterViewHolder.mDateView.setText(com.coderming.weatherwatch.Utility.getFriendlyDayString(mContext, dateInMillis, useLongToday));

        // Read weather forecast from cursor
        String description = com.coderming.weatherwatch.Utility.getStringForWeatherCondition(mContext, weatherId);
        ...
        double high = mCursor.getDouble(com.coderming.weatherwatch.ForecastFragment.COL_WEATHER_MAX_TEMP);
        String highString = com.coderming.weatherwatch.Utility.formatTemperature(mContext, high);
        ...
        double low = mCursor.getDouble(com.coderming.weatherwatch.ForecastFragment.COL_WEATHER_MIN_TEMP);
        String lowString = com.coderming.weatherwatch.Utility.formatTemperature(mContext, low);
